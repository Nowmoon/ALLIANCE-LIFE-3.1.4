...
//	if (BTC_def_hud == 1) then {player addAction [("<t color=""#00ff00"">" + ("Hud On\Off") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], -8, false, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
	if (BTC_def_pip == 1) then {player addAction [("<t color=""#00ff00"">" + ("Camera On\Off") + "</t>"),BTC_dir_action, [[],BTC_l_camera], -9, false, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip && (missionNamespace getVariable[""license_civ_dep"",false])"];};
	player addAction [("<t color=""#ffff00"">" + ("Helitreuiller") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check && (missionNamespace getVariable[""license_civ_dep"",false])"];
	player addAction [("<t color=""#ED2744"">" + ("Larguer le vehicule") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1 && (getPosATL player select 2) < 20 && (missionNamespace getVariable[""license_civ_dep"",false])"];
...
//			if (BTC_def_hud == 1) then {player addAction [("<t color=""#ED2744"">" + ("Hud On\Off") + "</t>"),BTC_dir_action, [[],{if (BTC_Hud_Cond) then {BTC_Hud_Cond = false;} else {BTC_Hud_Cond = true;_hud = [] spawn BTC_fnc_hud;};}], -8, false, false, "", "(vehicle player) isKindOf ""Helicopter"" && driver (vehicle player) == player"];};
			if (BTC_def_pip == 1) then {player addAction [("<t color=""#ED2744"">" + ("Camera On\Off") + "</t>"),BTC_dir_action, [[],BTC_l_camera], -9, false, false, "", "typeOf (vehicle player) in BTC_l_def_veh_pip && (missionNamespace getVariable[""license_civ_dep"",false])"];};
			player addAction [("<t color=""#ED2744"">" + ("Helitreuiller") + "</t>"),BTC_dir_action, [[],BTC_attach_cargo], 9, true, false, "", "[] call BTC_lift_check && (missionNamespace getVariable[""license_civ_dep"",false])"];
			player addAction [("<t color=""#ED2744"">" + ("Larguer le vehicule") + "</t>"),BTC_dir_action, [[],BTC_detach_cargo], -9, true, false, "", "BTC_lifted == 1 && (getPosATL player select 2) < 20 && (missionNamespace getVariable[""license_civ_dep"",false])"];
		};
	}];
};